...
namespace CP.VPOS.Banks.Payten
{
    internal class PaytenVirtualPOSService : IVirtualPOSService
    {
        private readonly string _urlAPITest = "https://entegrasyon.asseco-see.com.tr/msu/api/v2";
        private readonly string _urlAPILive = "https://merchantsafeunipay.com/msu/api/v2";

        private readonly string _url3Dtest = "https://entegrasyon.asseco-see.com.tr/msu/api/v2/post/sale3d/{0}";
        private readonly string _url3DLive = "https://merchantsafeunipay.com/msu/api/v2/post/sale3d/{0}";
        ...
        public SaleResponse Sale(SaleRequest request, VirtualPOSAuth auth)
        {
            ...
            if (!String.IsNullOrWhiteSpace(request.saleInfo.campaignCode))
            {
                var campaignList = this.AdditionalInstallmentQuery(new AdditionalInstallmentQueryRequest { saleInfo = request.saleInfo }, auth);

                if (campaignList.confirm == true && campaignList.installmentList != null && campaignList.installmentList.Any())
                {
                    var activeCampaign = campaignList.installmentList.FirstOrDefault(s => s.campaignCode == request.saleInfo.campaignCode);

                    if (activeCampaign != null)
                    {
                        req.Add("CAMPAIGNS", "[{\"campaignCode\": \"" + activeCampaign.campaignCode + "\", \"campaignName\": \"(" + activeCampaign.count + ") " + activeCampaign.campaignName + "\"}]");
                        req.Add("CAMPAIGNCODE", activeCampaign.campaignCode);
                    }
                }
            }
        ...
        public BINInstallmentQueryResponse BINInstallmentQuery(BINInstallmentQueryRequest request, VirtualPOSAuth auth)
        {
            ...
            if (dic["responseCode"].cpToString() == "00" && dic.ContainsKey("installmentPaymentSystem"))
            {
                ...
                if (keyValuePairs != null && keyValuePairs.Any())
                {
                    ...
                    {
                        Dictionary<string, object> com = JsonConvertHelper.Convert<Dictionary<string, object>>(JsonConvertHelper.Json<object>(item["commission"]));

                        response.installmentList.Add(new
                        installment
                        {
                            count = item["count"].cpToInt(),
                            customerCostCommissionRate = com["rate"].cpToSingle()
                        });
                    }
                }
            }
        ...
        {
            AdditionalInstallmentQueryResponse response = new AdditionalInstallmentQueryResponse();
            ...
            Dictionary<string, string> req = new Dictionary<string, string>
            {
                ...
                {"MERCHANTPASSWORD", auth.merchantPassword },
                {"MERCHANT", auth.merchantID },
                {"BIN", request.saleInfo.cardNumber.Substring(0,6) }
            };
            ...
            if (dic["responseCode"].cpToString() == "00" && dic.ContainsKey("campaignCodeList"))
            {
                response.confirm = true;

                List<Dictionary<string, object>> keyValuePairs = JsonConvertHelper.Convert<List<Dictionary<string, object>>>(JsonConvertHelper.Json<object>(dic["campaignCodeList"]));

                if (keyValuePairs != null && keyValuePairs.Any())
                {
                    response.installmentList = new List<AdditionalInstallment>();

                    foreach (var item in keyValuePairs.Where(s => s.ContainsKey("campaignType") == true && s["campaignType"].cpToString() == "ADDITIONALINSTALLMENT"))
                    {
                        bool applyForCreditCard = (bool)item["applyForCreditCard"];
                        bool applyForBusinessCard = (bool)item["applyForBusinessCard"];
                        decimal minumumAmount = item["minumumAmount"].cpToDecimal();
                        int baseInstallment = item["baseInstallment"].cpToInt();
                        Enum.TryParse(item["currency"].cpToString(), out Currency currency);


                        if (
                            request.saleInfo.installment >= baseInstallment &&
                            request.saleInfo.amount >= minumumAmount &&
                            request.saleInfo.currency == currency
                            )
                        {
                            response.installmentList.Add(new AdditionalInstallment
                            {
                                count = item["campaignInstallment"].cpToInt(),
                                campaignCode = item["campaignCode"].cpToString(),
                                campaignName = item["campaignName"].cpToString(),
                                campaignDescription = item["details"].cpToString(),
                                required = false
                            });
                        }
            ...
            try
            {
                var onlineCampaignList = AdditionalInstallmentQueryOnline(request, auth);

                if (onlineCampaignList != null && onlineCampaignList.Any())
                {
                    response.installmentList = response.installmentList ?? new List<AdditionalInstallment>();
                    foreach (var item in onlineCampaignList)
                        response.installmentList.Add(item);
                }
            }
            catch
            {
        ...
        private List<AdditionalInstallment> AdditionalInstallmentQueryOnline(AdditionalInstallmentQueryRequest request, VirtualPOSAuth auth)
        {
            var activePaymentlist = ActivePaymentSystem(auth);

            if (activePaymentlist == null || !activePaymentlist.Any())
                return null;

            var binDetail = BinDetailQuery(request.saleInfo.cardNumber, auth);

            if (binDetail == null || !binDetail.ContainsKey("paymentSystem"))
                return null;

            Dictionary<string, object> binPaymentSystem = JsonConvertHelper.Convert<Dictionary<string, object>>(JsonConvertHelper.Json<object>(binDetail["paymentSystem"]));

            if (!binPaymentSystem.ContainsKey("type") || !activePaymentlist.Any(s => s["type"].cpToString() == binPaymentSystem["type"].cpToString()) || !_onlineBankQuery.Any(s => s == binPaymentSystem["type"].cpToString()))
                return null;

            List<AdditionalInstallment> response = null;
            ...
            Dictionary<string, string> req = new Dictionary<string, string>
                {
                    ...
                    {"CUSTOMER", "1" }, //TODO: CUSTOMER
                    {"MERCHANTPAYMENTID", binPaymentSystem["type"].cpToString() },
                    {"NAMEONCARD", request.saleInfo.cardNameSurname},
                    {"CARDPAN", request.saleInfo.cardNumber},
                    {"CARDEXPIRY", request.saleInfo.cardExpiryDateMonth.ToString("00") + "." + request.saleInfo.cardExpiryDateYear.ToString()},
                    {"CARDCVV",request.saleInfo.cardCVV},
            ...
            if (dic["responseCode"].cpToString() == "00" && dic.ContainsKey("campaigns"))
            {
                List<Dictionary<string, object>> keyValuePairs = JsonConvertHelper.Convert<List<Dictionary<string, object>>>(JsonConvertHelper.Json<object>(dic["campaigns"]));

                if (keyValuePairs != null && keyValuePairs.Any())
                {
                    response = new List<AdditionalInstallment>();

                    foreach (var item in keyValuePairs.Where(s => s.ContainsKey("campaignCode")))
                    {
                        response.Add(new AdditionalInstallment
                        {
                            count = 0,
                            campaignCode = item["campaignCode"].cpToString(),
                            campaignName = item["campaignName"].cpToString(),
                            campaignDescription = item["campaignExtra"].cpToString(),
                            required = true
                        });
                    }
                }
        ...
        private List<Dictionary<string, object>> ActivePaymentSystem(VirtualPOSAuth auth)
        {
            List<Dictionary<string, object>> response = new List<Dictionary<string, object>>();

            Dictionary<string, string> req = new Dictionary<string, string>
            {
                {"ACTION", "QUERYMERCHANTPAYMENTSYSTEMS" },
                {"MERCHANTUSER", auth.merchantUser },
                {"MERCHANTPASSWORD", auth.merchantPassword },
                {"MERCHANT", auth.merchantID },
            };
            ...
            if (dic.ContainsKey("responseCode") && dic["responseCode"].cpToString() == "00" && dic.ContainsKey("paymentSystems"))
            {
                List<Dictionary<string, object>> keyValuePairs = JsonConvertHelper.Convert<List<Dictionary<string, object>>>(JsonConvertHelper.Json<object>(dic["paymentSystems"]));

                foreach (var item in keyValuePairs)
                {
                    if (!response.Any(s => s.ContainsKey("type") && s["type"].cpToString() == item["type"].cpToString()))
                    {
                        response.Add(new Dictionary<string, object> {
                            {"name", item["name"].cpToString() },
                            {"type", item["type"].cpToString() }
                        });
        ...
        private string getSessionToken(SaleRequest request, VirtualPOSAuth auth)
        {
            ...
            if (dic["responseCode"].cpToString() == "00")
                ...
                throw new Exception("Payten session token oluşturulamadı");
        }
        ...
        private string getErrorDesc(string ErrCode)
        {
            ...
            switch (ErrCode)
            {
                ...
                case "ERR10147": desc = "3D doğrulama olmaksızın kart ekleme yetkiniz yoktur. Lütfen HPP entegrasyon modelini kullanarak kart ekleyiniz ya da Payten destek ekibinden yardım alınız."; break;
                ...
                case "ERR10153": desc = "İade işlemi Payten Finans ekibi tarafından red edilmiştir"; break;
                ...
                case "ERR10170": desc = "Girilen değer geçerli aralığın dışında. Minimum değer 1 olmalı, maksimum değer için lütfen Payten Operasyon Ekibi'yle iletişime geçiniz."; break;
        ...
        private Dictionary<string, object> BinDetailQuery(string binNumber, VirtualPOSAuth auth)
        {
            try
            {
                if (binNumber.cpIsNullOrEmpty() || binNumber.Length < 6)
                    return null;
                ...
                if (dic.ContainsKey("responseCode") && dic["responseCode"].cpToString() == "00")
                    return dic;
